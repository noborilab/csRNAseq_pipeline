...
| `P20ReadLength`, `P80ReadLength` | 20th and 80th percentiles of the same histogram, so the shape of the distribution can be read and not just its centre. Descriptive rather than diagnostic: size selection moves all three percentiles together, so a 40-70 nt input library and a 20-70 nt csRNA library can share a P80 while looking nothing alike, and the spread between them does not separate library types either. Against its own matched input a csRNA library did carry the longer upper tail in 11 of 12 pairs on the protocol-development panel, by 1 to 4 nt, so the comparison is worth making within a pair and not across a table. |
| `ModeReadFraction` | Percentage of reads at `ModeReadLength`, i.e. how tall the tallest bin is. This is the shape column that separates: on the protocol-development panel every passing csRNA library sat between 4.8 and 8.6, every failing one between 9.9 and 15.5, and every input library between 10.9 and 26.1, with no overlap. A library made of one processed species concentrates in a few lengths; initiation spread over real promoters does not. Not gated on, and not independent evidence: it runs at r = -0.92 against `PctNuclear` and -0.74 against `csFRiP` on the same panel, so it largely restates gates the table already has. |
| `ModeReadLength` | Commonest read length in the same histogram, smaller length winning a tie. Weak on its own: a healthy csRNA library has a nearly flat length distribution, top bin around 5 to 6 per cent, so the winning length flips between replicates of the same arm. It earns its place on the libraries that are not flat, where a discrete population takes a much larger share (22 per cent at 24 nt in an input library, 15 per cent at 30 nt in a phosphatase-free arm) and the mode lands on it. Treat a mode far from the median as a prompt to look at the histogram, not as a measurement. |
| `TSSDistinctFrag` | Distinct fragments observed inside the final TSS set, a fragment being a `(position, strand, length)` triple from the tag directory, so both ends define it. Depends on sequencing depth, so it is the denominator for the two below rather than something to compare across libraries. Organelle chromosomes are excluded, as for `csFRiP`. |
| `TSSChao1` | Chao1 extrapolation of the fragments the library could yield at infinite depth, from its singleton and doubleton counts. Needs no subsampling, which is why it is reported: on the protocol-development panel the shallowest library had 2.5M in-TSS reads, so a fixed-depth subsample would have to discard 90% of a good library's data or return `NA` for the libraries most worth judging. Among libraries that pass the other gates it tracks a depth-matched count at rho = 0.88. |
| `TSSSaturation` | Good's coverage, `1 - singletons/reads`: the estimated chance that the next read comes from a fragment already seen. The number to read when deciding whether more sequencing would pay. On the RNA input series it ran 0.936 at 50 µg and 0.994 at 1 µg, so it separates an exhausted library from one with more to give. |
