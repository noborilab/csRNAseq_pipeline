...
| `P20ReadLength`, `P80ReadLength` | 20th and 80th percentiles of the same histogram, so the shape of the distribution can be read and not just its centre. Neither means much alone, because size selection moves all three percentiles together: a 40-70 nt input library and a 20-70 nt csRNA library can share a P80 while looking nothing alike. `P80ReadLength` minus `MedianReadLength` is the number to look at. A csRNA library carries a longer upper tail than the input made from the same RNA, which is what a capped, un-processed population looks like against a background of processed fragments. |
